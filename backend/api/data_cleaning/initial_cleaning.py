...
def merge_and_process_data(
        ...
        output_file='data_to_clean/1_merged_typed_data.parquet'
):
    """Main data merging and processing pipeline."""
    # Remove all files from data_to_clean if not empty

    if input_files is None:
        input_files = [
            os.path.join('data_to_clean', f)
            for f in os.listdir('data_to_clean')
            if os.path.isfile(os.path.join('data_to_clean', f))
        ]
